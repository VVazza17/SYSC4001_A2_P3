 ...
 *
 * @file interrupts.cpp
 * @author Sasisekhar Govind, Kyle Deng
 *
 */

#include "interrupts_101299542.hpp"
...
std::tuple<std::string, std::string, int> simulate_trace(std::vector<std::string> trace_file, int time, std::vector<std::string> vectors, std::vector<int> delays, std::vector<external_file> external_files, PCB current, std::vector<PCB> wait_queue) {
    ...
    for(size_t i = 0; i < trace_file.size(); i++) {
        ...
        if(activity == "CPU") { //As per Assignment 1
            ...
            // FORK ISR body: clone, snapshot and run child to completion
            execution += std::to_string(current_time) + ", " + std::to_string(duration_intr) + ", cloning the PCB\n";
            current_time += duration_intr;

            // parent waits, child runs
            PCB parent_snap = current;
            PCB child(current.PID + 1, current.PID, current.program_name, current.size, -1);
            wait_queue.push_back(parent_snap);
            current = child;

            // allocate memory for the child so it occupies a real partition (expected: partition 5)
            allocate_memory(&current);

            // scheduler and  IRET to finish ISR
            execution += std::to_string(current_time) + ", 0, scheduler called\n";
            execution += std::to_string(current_time) + ", 1, IRET\n";
            current_time += 1; // move past IRET

            // snapshot after FORK (after IRET => time matches expected 24)
            ...
            //  restore parent as current and pop wait queue
            current = parent_snap;
            wait_queue.pop_back();
            ///////////////////////////////////////////////////////////////////////////////////////////
            ...
            // look up size, simulate loader, mark memory, update PCB
            unsigned int sz = get_size(program_name, external_files);
            execution += std::to_string(current_time) + ", " + std::to_string(duration_intr) + ", Program is " + std::to_string(sz) + " Mb large\n";
            current_time += duration_intr;

            int load_ms = static_cast<int>(sz) * 15;
            ...
            current.size         = sz;
            // reuse partition selection logic; only free if already allocated
            ...
            // status snapshot after EXEC (after IRET => times 247 / 620)
            system_status += "time: " + std::to_string(current_time) + "; current trace: EXEC " + program_name + ", " +
                             std::to_string(duration_intr) + "\n";
            system_status += print_PCB(current, wait_queue);
            ///////////////////////////////////////////////////////////////////////////////////////////

            // open external program trace (from input_files folder)
            std::ifstream exec_trace_file(std::string("input_files/") + program_name + ".txt");
